...
Not yet built (next phases): VaultFactory, a fork test asserting the whole wiring, the AuctionHouse switch from `capPrice` to `referencePrice` (OQ-005), and a `renounceOwnership` override on `CoveredCallVault` — the only owned contract still missing one, and a renounce would now brick `injectCoverage` (D-099). **`CoveredCallVault.injectCoverage` ships in v0.7 (D-099)**, so the slash-to-shortfall loop closes on chain: the timelock raises `payoutPerOption` for the still-unclaimed options of a shortfall series back to its unscaled value, pulling only what that raise makes claimable (SPEC §14; `coverageNeeded(seriesId)` sizes the purchase). The FeeRouter WRITE mode and the WRITE bond migration are implemented as of v0.6 but stay dormant until the timelock wires the token.
