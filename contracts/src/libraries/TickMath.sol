...
/// @title TickMath
/// @notice `sqrt(1.0001^tick) × 2^96` for Uniswap v3 ticks (SPEC §9.5). The constants are those of Uniswap
/// v4-core `TickMath.getSqrtPriceAtTick` (MIT); this port uses no assembly (CLAUDE.md rule 8, D-055).
library TickMath {
    ...
    function getSqrtRatioAtTick(int24 tick) public pure returns (uint160 sqrtPriceX96) {
        ...
        // Q128.128 → Q64.96, rounding up so that getTickAtSqrtRatio(getSqrtRatioAtTick(t)) == t (Uniswap convention).
