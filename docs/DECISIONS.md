...
## D-113 · 2026-09-05 · Internal audit pass: freeze the price source, derive `resolveRef` on-chain at the backstop, guard `OptionToken.claim`

**Context.** Mainnet opens with hard deposit caps and without an external auditor (docs/CAPS.md). An internal audit (docs/AUDIT-PREP.md: slither, aderyn, coverage, a long invariant campaign, four fresh-context reviews against SPEC and THREAT-MODEL) found one High-impact gap and a handful of Low ones.

**Decision.**
1. `AuctionHouse.freezePriceSource()` and `CapController.freezePriceSource()`: `onlyOwner`, one-way, refuse a source without code (so the deploy cannot lock onto the `0xdEaD` placeholder). Both are executed in deploy batch A immediately after the two `setPriceSource(settlementOracle)` calls, and `DeployChecks.assertCoreWiring` requires both flags. `S_ref` (strike, reserve bounds) and `S_cap` (every deposit cap) can then change only by redeploying the consumer. This closes the freeze half of OQ-005; the `referencePrice` switch stays a redeploy.
2. `SettlementOracle.halt`: when the hint verifies no reference round (only possible past the 7-day backstop, D-057), `resolveRef` is the hinted round if it is a valid round at or before expiry, else the feed's latest round if at or before expiry, else `sRef` — derived on-chain, so a caller who withholds the hint can force the halt (already accepted in D-057) but cannot choose the band's centre.
3. `OptionToken.claim` is `nonReentrant`, so THREAT-MODEL T-16's "every state-changing external function" holds literally rather than only through the vault's guard.
4. Both TWAP readers fail the path (`OBSERVE` / `TICK_RANGE`) when the averaged tick is outside `[MIN_TICK, MAX_TICK]`, before `TickMath.getSqrtRatioAtTick` could revert outside the `observe` try/catch; `WritePriceOracle` takes the cumulative deltas `unchecked` like `SettlementOracle` already did.
5. A `deep` Foundry profile (`out-deep/`, `runs = 6000`, `depth = 128`) for long invariant campaigns, and every static-analysis finding triaged in docs/AUDIT-PREP.md.

Second round (the three focused reviews, same day):
6. **Carried redeems (F-1).** `openSeries` runs `_processRedeems(maxQueueOpsPerOpen)` before `_processDeposits` and reverts `CannotOpen("REDEEM_QUEUE")` (also reported by `canOpenAuction`) while any request is still QUEUED. Excluding a carried request from `offeredQty` never excluded it from the pooled NAV: the redeemer bore the next series' payout with no premium (worked example in AUDIT-PREP §4). `requestRedeem` reverts in IDLE, so the queue cannot be refilled before an open and the permissionless `processRedeems` drains anything longer than one open visits. This also closes the queue-then-cancel offer-shrinking grief (F-3).
7. **Late clears (F-2).** `AuctionHouse.clearGrace` (1 h default, `[5 min, 4 h]`, `setClearGrace`): `_compute` takes the skip path at `auctionClose + clearGrace`. Bids cannot be cancelled and depositors cannot exit during AUCTION, so an open-ended clearing window was a free option for every bidder whenever the keeper was down. A skipped week is the cheaper failure. The keeper clears at `auctionClose`; the handler jitter of ≤ 30 min stays inside the default.
8. **Verified observation hint (R-1).** `_observationsOk` reverts `BadObsHint` for a hint that is uninitialised, after the anchor, or followed by a newer observation still at or before the anchor. The advisory rule of D-053 let the live head as hint fail path 2 and pick path 3 on a weekend series once Monday's round existed. A correct hint always exists when `observe` succeeded; the test fixture's default `_hint` now computes it like the keeper does.
9. **Phase overlap (R-2)** and **absurd answers (R-5)** in SettlementOracle: a phase-`p` reference is rejected when `(p+1, 1)` exists at or before expiry (and a phase-`p` first-after round when `(p+1, 1)` is earlier), so an aggregator upgrade never offers two verifiable candidates; an answer above `uint128.max` is treated as invalid instead of reverting `halt` inside `toUint128`.
10. **Zero-asset redeem (N-1)** refused on `redeem`/`withdraw`; **`claimPremium` capped at balance (R-3)**; the tick-range guard checks the floored tick, not the truncated average (R-4); the last unstaker takes the whole principal so a slash cannot leave `totalStaked > 0` behind zero shares, and a near-total exit may not take the whole principal either (the virtual `+1` asset rounded `previewUnstake(totalShares − 1)` up to everything, leaving a worthless share) — both halves of I-21, both found by the deep campaign.
11. **Wiring asserts (C-2, C-3).** `OptionToken.registerVault` asserts `vault.stock() == underlying` and `vault.optionToken() == this`; `BondManager.setAuctionHouse`, `FeeRouter.setAuctionHouse` and `RiskModule.setSettlementOracle` probe the target's back-pointer. Outside deploy batch A nothing else re-asserted these one-shot, irreversible pointers.
12. **Mainnet governance shape pinned (C-1, C-4, C-5, C-7).** `Config.validate` on chainId 4663 requires `timelockMinDelay ≥ 48 h`, `deployerIsAdmin == false`, exactly two guardians and pairwise-distinct admin/keeper/guardian keys; `DeployChecks` asserts the 48 h delay on 4663 and the curator floors per vault; `Deploy._governanceIsUs` needs the declared deviation, not just a zero delay; `DeployToken` refuses a `TIMELOCK_ADDRESS` that is not a ≥ 48 h timelock; `deploy.sh` is Ledger-only on 4663 and no longer exports the whole `.env` into child processes.

**Accepted / documented rather than changed (this round).** `Ownable2Step.transferOwnership` on every contract (a 48 h step can hand a contract to an undelayed key; watch `OwnershipTransferStarted`, THREAT-MODEL T-14 item 3); `Deploy.run()` is not idempotent on a real-delay chain (RUNBOOK: do not re-run between stage 1 and batch A); `CapController.setCapUSD` has no upper bound (a compromised timelock can already do worse; docs/CAPS.md is the policy); the jump-guard multiplier exception (D-051) stays as specified; the T-12.5 80 h USDG staleness in WritePriceOracle stays (band-bounded); `_processDeposits` may mint zero shares at an extreme share price (standard ERC-4626 offset behaviour, T-06).

**Alternatives considered.** (1) Deriving `priceSource` from the vault's immutable `settlement` and deleting the setter — rejected for now: it changes the constructor/fixture shape of every auction test and the deploy ordering, which is the OQ-005 redeploy itself; the freeze gives the same guarantee today at ~150 bytes. Cross-checking `sRef` in `openAuction` against the vault's own oracle — redundant once the source cannot move. (2) Leaving `resolveRef = sRef` and documenting it — rejected: a caller-selectable band centre is exactly the kind of discretion D-022 exists to remove, and the fix is a view over data already read. (3) Amending T-16's text instead of adding the guard — rejected: the guard costs one storage slot and closes the question for any future hooked stock token.

**Consequences.** SPEC v0.8 (§3, §4.3, §7.2, §8.2, §9.1, §9.3, §9.5, §9.6, §13, §14, §16.1, §16.2, §18), THREAT-MODEL T-08, T-13, T-14 (item 3b), T-16, T-18; RUNBOOK §3 (batch A is 19 calls) and §2 (Ledger-only on 4663); keeper: always supply `obsIndex` (newest observation at or before expiry) and clear within `clearGrace`; `test_mainnetBatchAIsTheSizeTheRunbookQuotes` re-pinned; new tests `test_halt_backstop_withheldHintDoesNotMoveResolveRef`, `test_halt_backstop_nonLastHintStillAnchorsResolveRef`, `test_path2_reason_OBSERVE_onOutOfRangeTick`, `testFuzz_path2_neverRevertsOnArbitraryCumulatives`, `test_writePrice_notOkOnOutOfRangeTick`, freeze branches in `AuctionHouse.t.sol` / `CapController.t.sol`. The testnet 46630 system predates this and must be redeployed before it is used as a rehearsal of batch A.
