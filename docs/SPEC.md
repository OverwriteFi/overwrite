...
- `refRound` = the last valid Chainlink round with `updatedAt ≤ expiry`, **any age** (D-021, closes T-08.2). It is the reference for every TWAP sanity bound (§9.2 fallback, §9.3) and for the resolution band (§9.6). It is deterministic: it does not depend on when `settle` is called. The keeper supplies it as a hint; the contract verifies it with `next(refRound).updatedAt > expiry` (or none), skipping at most 8 successors with `answer ≤ 0` (D-053). `refRoundId = 0` ("no valid round") is accepted only when the feed's first round `(1 << 64) | 1` is after expiry or the feed is unreachable; otherwise `RefRoundRequired`. If no valid round exists (feed dead since before expiry) `refRound` is undefined and the bounds that need it fail, i.e. the TWAP paths are unavailable and `sRef` is the resolution reference.
...
- `oracle.isLastRoundAtOrBefore(vault, roundId, ts)`, `oracle.isFirstRoundAfter(vault, roundId, prevRoundId, ts)`, `oracle.lastChainlink(vault)` views used to build hints; `oracle.canHalt(seriesId, hint) → (bool, reason)`; `oracle.twap(vault, window) → (twapUSDG8, twapUSD8, reason)`
