...
Version 0.8 · 2026-09-05 · Status: **internal audit pass (docs/AUDIT-PREP.md, D-113)**: `AuctionHouse.freezePriceSource` / `CapController.freezePriceSource` (one-way, in deploy batch A), on-chain `resolveRef` fallback at the halt backstop (§9.6), `nonReentrant` on `OptionToken.claim`, tick-range guard before `getSqrtRatioAtTick` in both TWAP readers; `openSeries` drains the redeem queue before offering (§4.3, closes the carried-redeem loss); `clear` skips after `auctionClose + clearGrace` (§8.2, closes the late-clear free option); the observation hint is verified, not advisory (§9.3, T-08); phase-overlap rounds have one verifiable reference (§9.1); a redeem that would pay zero is refused; `claimPremium` is capped at the balance; wiring asserts on `OptionToken.registerVault` and the three set-once back-references; the mainnet governance shape is pinned in `Config.validate`; and the doc drift listed in AUDIT-PREP §4 corrected. Previous: Version 0.7 · 2026-09-04 · Status: vault, auction, settlement and **token** layers implemented (`contracts/src/CoveredCallVault.sol`, `OptionToken.sol`, `CapController.sol`, `AuctionHouse.sol`, `BondManager.sol`, `FeeRouter.sol`, `SettlementOracle.sol`, `RiskModule.sol`, `WRITE.sol`, `SafetyModule.sol`, `EmissionsController.sol`, `WritePriceOracle.sol`, `Vesting.sol`, `PointsDistributor.sol`, `LiquidityEscrow.sol`), VaultFactory draft. v0.7 records `CoveredCallVault.injectCoverage` (D-099), which closes the slash-to-shortfall loop on chain (§14): the timelock raises `payoutPerOption` for the still-unclaimed options of a shortfall series back to the unscaled payout recomputed from the stored settlement price and strike, pulling only what that raise makes claimable, with `OptionToken.raisePayout` keeping the published mirror in step and `coverageNeeded` sizing the purchase. v0.6 records the WRITE token layer (D-059…D-096): the fixed supply minted into five distribution contracts with an `allocation()` handshake (§14), the share-based safety module and its pulled emissions (§14), the shared `WritePriceOracle` behind `safetyModuleValueUSD` (§12), the dual-asset bond migration (§13), and the WRITE fee path running inside `flush` (§11). v0.5 records the settlement layer (D-050…D-056): oracle parameters live as versions in RiskModule selected by `auctionOpen` (§6), the jump-guard OR semantics (§9.1), the orientation-specific depth rule and price formula (§9.3, §9.5), the hint rules, exhaustion proofs and the 7-day halt backstop (§9.1, §9.6), `capPrice` vs `referencePrice` (§7.2, §12), the settlement invariants (§17). v0.4 records what changed while implementing the auction (D-043…D-048): exact escrow conservation (`premiumGross` = Σ floored payments, §7.3, §8.2, I-3), set-once wiring of BondManager/FeeRouter (§3), the skip paths of `clear` (§8.2 step 8), `auctionOpen = block.timestamp` and the Friday-anchored weekend window (§5), `S_ref == S_cap` until SettlementOracle ships (§7.2), the pro-rata dust carry (§8.2 step 4), read functions (§16.2) and the auction invariants (§17). v0.2 applied the sixteen revisions RS-01…RS-16 from THREAT-MODEL.md, recorded as DECISIONS.md D-018…D-034 (RS-03 is folded into D-019; D-028 records the `KEEPER_ROLE` gate; D-034 records `sunset`). v0.3 also folds in the independent review of the vault layer (D-040…D-042: coverage check vs `totalAssets`, queue-processing hardening, retained `effectiveAt`, `Ownable2Step`) and records what changed while implementing the vault (D-035…D-039): the vault ↔ AuctionHouse / SettlementOracle / OptionToken call graph (§3), explicit `encumbered` and saturating `totalAssets` (§4.1), premium accumulator precision 1e36 (§4.2), redeem escrow and `EXPIRED` deposit requests (§4.3), the split of the Series struct between vault and AuctionHouse (§6), vault-only option minting (§8.2 step 6), new events (§16.1), invariant I-16 (§17).
...
Libraries: OpenZeppelin 5.1 (`ERC4626`, `ERC1155`, `TimelockController`, `AccessControl`, `ReentrancyGuard`, `SafeERC20`), Chainlink `AggregatorV3Interface`, Uniswap v3 `TickMath` / `FullMath` / `OracleLibrary`-equivalent (re-implemented under solc 0.8.26 without assembly beyond the audited library). No upgradeable proxies in v1; a v2 is a new deployment plus migration via `sunset` (§15, D-034). Every cross-contract reference is `immutable` or **set once** by the timelock and asserted by the counter-party (D-044, D-049): `BondManager.auctionHouse`, `FeeRouter.auctionHouse`, `RiskModule.setSettlementOracle`, `EmissionsController.setWriteToken/setSink`, `setWriteToken` on the five WRITE holders, `FeeRouter.setWriteToken`, `BondManager.setWriteToken`. `AuctionHouse.priceSource` and `CapController.priceSource` are settable until `freezePriceSource` is called, which deploy batch A does right after pointing both at the SettlementOracle (D-039, D-047, D-054, **D-113**); after that they are as fixed as an immutable. The remaining *re-settable* references are deliberate liveness escapes recorded per contract: `SafetyModule.setOracle` (D-078), `FeeRouter.setPriceOracle` (D-039 precedent), `CapController.setSafetyModule` (D-041), `WritePriceOracle.setChainlinkFeed/setPool/setSequencerFeed` (D-064…D-068), `LiquidityEscrow.setPool` until the first `fund` (D-094).
...
- `redeem`/`withdraw` refuse a request that would pay zero assets (`ZeroAmount`): after an issuer burn `totalAssets()` can saturate at zero while shares exist, and OpenZeppelin would otherwise burn the shares for nothing; refusing keeps them alive for a coverage injection (audit N-1, D-113). `claimPremium` pays `min(claimable, USDG balance)` and keeps the remainder credited, so accumulator dust from balance splits can never brick the last claimant (audit R-3).
- Queue processing is part of the settlement transaction up to `maxQueueOpsPerSettle` (default 50) and continues permissionlessly afterwards. `openAuction` first executes up to `maxQueueOpsPerOpen` (default 50) queued deposits and then opens **regardless of any remaining queue** (D-032, closes THREAT-MODEL T-18); tokens still queued at open are not in `totalAssets()`, are not in `offeredQty`, and execute at the next IDLE. Redeem requests still queued when the next auction opens are **executed by `openSeries` first** (up to `maxQueueOpsPerOpen`, at the post-settlement share price), and the open is refused (`CannotOpen("REDEEM_QUEUE")`, reported by `canOpenAuction`) while any request is still QUEUED, so the offer is exactly the assets of the shares that stay (audit F-1, D-113). Merely excluding a carried request from `offeredQty` (the v0.3–v0.7 rule) did not exclude it from the pooled NAV, so a carried redeemer bore the next series' payout without earning its premium. `requestRedeem` reverts in IDLE, so the queue can only shrink before an open and this cannot be griefed; the permissionless `processRedeems` clears a queue longer than one open can visit.
...
struct Series {
  ...
  OracleParams params;      // snapshot of the vault's oracle parameters at openAuction (D-031); immutable for the life of the series.
                            // v0.5: not stored per series — RiskModule keeps append-only versions per vault and SettlementOracle reads
                            // `paramsAt(vault, auctionOpen)`; a version set in block t applies from t + 1 (D-050, D-053)
}
```
...
- `S_ref` is read on-chain, never keeper-supplied:
  ...
  **As deployed (OQ-005 open):** the AuctionHouse reads `S_ref` through the same `IPriceSource.capPrice(vault)` as the CapController (`S_ref == S_cap`: Chainlink < 80 h, no `oraclePaused` check, else the 30-min TWAP; D-047, D-054), not through `SettlementOracle.referencePrice`, which implements items 1–3 above but is not yet consumed. The source is frozen by `freezePriceSource` in deploy batch A (D-113), so switching to `referencePrice` is an AuctionHouse redeploy. A zero or unavailable price reverts `NoReferencePrice`, a price below 400 (8 dec) reverts `GridZero`.
...
8. Skip path (D-045), taken when there is no bid, when `remaining == 0`, when `now ≥ expiry` (keeper down for the whole series), when `now ≥ auctionClose + clearGrace` (**default 1 h, timelocked within [5 min, 4 h]**; audit F-2, D-113 — bids cannot be cancelled and depositors cannot exit during AUCTION, so an open-ended clearing window gave every bidder a free option on the underlying's move for up to five days whenever the keeper was down: clear if the stock rallied past the strike, let it skip otherwise), or when the premium would be non-zero but every share is escrowed for redeem (the vault would revert `NoSharesForPremium`): series → SKIPPED, vault → IDLE, `AuctionSkipped` emitted, all bond locks released, all escrow credited to `refundable`. The vault therefore never stays in AUCTION once `clear` is callable. `previewClear` reports `willSkip`.
...
- `refRound` = the last valid Chainlink round with `updatedAt ≤ expiry`, **any age** (D-021, closes T-08.2). It is the reference for every TWAP sanity bound (§9.2 fallback, §9.3) and for the resolution band (§9.6). It is deterministic: it does not depend on when `settle` is called. The keeper supplies it as a hint; the contract verifies it with `next(refRound).updatedAt > expiry` (or none), skipping at most `MAX_GARBAGE_SKIP = 32` successors with `answer ≤ 0` (D-053, D-057). `refRoundId = 0` ("no valid round") is accepted only when the feed's earliest round — probed and stored at `registerVault` — is after expiry *and* the feed's own `latestRoundData()` is not a valid round at or before expiry; otherwise `RefRoundRequired` (D-057). A run of invalid rounds longer than the skip bound leaves every hint unverifiable; §9.6's backstop is then the only way forward, by design. If no valid round exists (feed dead since before expiry) `refRound` is undefined and the bounds that need it fail, i.e. the TWAP paths are unavailable and `sRef` is the resolution reference.
- **Jump guard (D-025, closes T-02.1, T-10.2, T-14.2).** Every candidate settlement price `S` on every path (1, 2, 3) must satisfy `|S / sRef − 1| ≤ jumpBps` (per vault, default **3 000 bps = 30 %**, timelocked within `[1 000, 5 000]`, snapshotted at open), where `sRef` is the reference spot stored at `openAuction` (§7.2). Exception, *a multiplier change explains it*: if `token.uiMultiplier()` at settle time `m_now ≠ multiplierAtOpen` and `token.oraclePaused() == false`, the guard also accepts `S` when `|S × multiplierAtOpen / m_now / sRef − 1| ≤ jumpBps` (plain check OR adjusted check, D-051, matching I-10). Note for implementers and auditors: under ERC-8056 the feed prices one raw token as share price × multiplier, so a correctly sequenced corporate action does **not** move `S` and passes the plain check; the exception only admits a price that moved by the multiplier ratio, which is exactly the feed/multiplier mis-sequencing of THREAT-MODEL T-10.2. Because D-026 refuses to open a series across a staged change, the exception can only apply to a change staged after open; the keeper alerts on `UIMultiplierUpdated` inside a live series and the guardian pauses new auctions until the settlement has been reviewed. A price rejected by the guard → `JumpGuardTripped(seriesId, path, S)`; the path is treated as failed; if all paths fail → HALTED with `reason = JUMP_GUARD` (§9.6).
...
- Phase-aware neighbour lookup: for proxy round `r = (p << 64) | a`, `next(r)` is `(p << 64) | (a+1)` if it exists, else `((p+1) << 64) | 1` if that exists, else none. `prev(r)` is `(p << 64) | (a−1)` if `a > 1`, else the last round of phase `p−1` supplied by the keeper in the hint and verified to have `next == r`. **Phase overlap (audit R-2, D-113):** while an old aggregator keeps publishing after a proxy upgrade, both phases can hold a "last round ≤ expiry" / "first round > expiry"; the contract accepts only the newest phase's candidate — a phase-`p` reference is rejected when `((p+1) << 64) | 1` exists with `updatedAt ≤ expiry`, and a phase-`p` first-after round when `((p+1) << 64) | 1` exists with an earlier `updatedAt`. A round whose `answer` exceeds `uint128.max` is treated like `answer ≤ 0` (audit R-5): it can never be a settlement price and would otherwise revert `halt` when it became `resolveRef`.
...
1. pool checks pass: `observe` succeeds for the anchored window (observation cardinality covers it); **time-weighted** in-range liquidity over the window is deep enough that a **250 000 USDG swap would move the price by less than 1 %** (founder decision D-010, revised by D-018 to use the window average instead of spot liquidity, closes T-03.3). `L_avg` is the harmonic-mean liquidity over the window from the same `observe` call (§9.5); `sqrtP` is `TickMath.getSqrtRatioAtTick(twapTick)`. Require
   ...
   - USDG = token1 (stock = token0; SPY, TSLA, AMZN, GOOGL by address order): `L_avg × (√1.01 − 1) × sqrtP_X96 / 2^96 ≥ 250 000e6` — buying token0 raises `sqrtP`, the token1 amount is `L × ΔsqrtP` (D-052; the v0.4 text claimed the same inequality for both orientations, which is off by `sqrtP² / 2^192`)
   with `√(1 + impactBps / 1e4) × 1e9` computed as `Math.sqrt((1e4 + impactBps) × 1e14)` (1 004 987 562 for 100 bps, i.e. `√1.01 − 1 = 4 987 562 / 1e9`); `swapNotionalUSDG` (250 000e6) and `impactBps` (100) are timelocked parameters snapshotted at open; pool is the vault's immutable 0.05 % pool. Spot `pool.liquidity()` is not used: an LP that pulls liquidity for the window and re-adds it before the call no longer passes;
   and **trading activity inside the window** (D-019, closes T-03.4 and T-05.4): at least `minObservationsInWindow` (default **3**, timelocked within `[1, 16]`) pool observations have `blockTimestamp` inside `[expiry − window, expiry]`, and the most recent observation at or before `expiry` is no older than `expiry − 900`. Checked by walking `pool.observations(i)` backwards from the keeper-hinted index of the newest observation at or before expiry (bounded to `minObservationsInWindow + 2` entries, wrapping at cardinality). **The hint is verified (audit R-1, D-113):** it must be initialised, at or before the anchor, and the next ring slot must not be a newer observation still at or before the anchor; otherwise `BadObsHint` reverts, exactly like a wrong round hint. The v0.5 rule ("an older hint only under-counts and fails the path", D-053) let anyone with the live head as hint fail the TWAP and settle a WEEKEND series on path 3 once Monday's first round existed — a free choice between two legitimate prices (T-08). If `observe` succeeded, the window start is inside the ring, so a correct hint always exists; the keeper computes it as the newest observation at or before expiry. A quiet pool or a sequencer outage across the window leaves no observations in it and the TWAP is rejected instead of settling on an extrapolated tick;
...
4. sequencer hook passes;
5. jump guard passes (§9.1).
Then `S = r.answer`, `settlementPath = 3`; as in §9.6, invalid rounds directly after expiry are skipped and the deadline applies to the round actually used (D-057). The 24/5 feed reopens 18:00 ET Sunday (22:00 or 23:00 UTC), and the first post-weekend round arrived at 00:00:33–00:00:54 UTC Monday on 2026-08-31 for both SPY and NVDA, so this path normally resolves within an hour of expiry.
...
```
`spl[1] − spl[0] == 0` (no time elapsed, impossible for `window > 0`) fails the path with reason `OBSERVE` (returned, not reverted; both deltas are taken `unchecked` because the cumulatives wrap by design, and an average tick outside `[MIN_TICK, MAX_TICK]` also fails with `OBSERVE` before `getSqrtRatioAtTick` can revert, audit A-01). `L_avg` replaces `pool.liquidity()` in the depth rule of §9.3. Observation-activity rule: see §9.3 item 1 (D-019); the observations are read directly from `pool.observations(index)`.
...
- if stock token is `token0`: `price8 = 1e8 × 1.0001^tick × 10^(dS − dU)`; computed as `mulDiv(1e8 × 1e18, sqrtP², 2^192) / 1e6` (D-052 fixed the inverted decimal factor of v0.4).
Check against cast: tick 222534 → `1e12 / 1.0001^222534 = 216.75`, feed answer 216.79. ✓ (`test_tickMath_knownValues`)
...
- `PathRejected(seriesId, 2, reason)` is emitted with `USDG_STALE` or `USDG_OUT_OF_BAND` so the keeper and frontend can show why the primary path was skipped.
...
- `settle` reverts with a typed error until the keeper (or anyone) calls `halt(seriesId)` once all paths are provably exhausted on-chain (for WEEKDAY: no valid round and `now > expiry + twapGrace`, or every available round tripped the jump guard; for WEEKEND: `now > expiry + 54 060`, or the same). A USDG depeg alone never halts a weekend series; it only removes the TWAP path (§9.5). `halt` sets `state = HALTED`, calls `RiskModule.pauseNewAuctionsOnHalt(vault, seriesId, reason)`, emits `SeriesHalted` with `reason ∈ {NO_ORACLE_PATH, JUMP_GUARD}` and fixes `resolveRef`. `halt` is refused while `oraclePaused()` or the sequencer hook fails, and while path 3 is still available (a weekend round inside the Monday 15:00 deadline settles however late `settle` is called). **Backstop (D-053 as amended by D-057):** from `expiry + haltedTimeout` no hint failure reverts and an unusable hint counts as "no path", so a feed that never unpauses, a dead feed or a run of invalid rounds longer than the skip bound can never lock a vault. The backstop never pre-empts a path that still succeeds with the supplied hint, so a keeper outage cannot be converted into a different settlement price by whoever calls first; a caller who withholds a usable hint can still force the halt, which is bounded by the resolution band below.
- **Resolution band (D-022, closes T-14.2).** `resolveRef` = `refRound.answer` (§9.1: the last valid Chainlink round at or before expiry, any age). If the hint did not verify one — only possible past the 7-day backstop, where an unusable hint no longer reverts (D-057) — the centre is still derived on-chain and never chosen by the caller (**D-113**, audit G-2): the hinted round if it is a valid round at or before expiry (even when not provably the last), else the feed's latest round if that is at or before expiry, else `sRef`. `resolveBoundBps = 2 500` is a **contract constant**, not a parameter. Any resolution price must satisfy `resolveRef × 0.75 ≤ price8 ≤ resolveRef × 1.25`.
...
- `resolveHaltedByOracle(seriesId, roundHint)` — **permissionless after `haltedTimeout = 7 days` past `expiry`** (D-033, closes T-14.7 and T-11.6). Accepts the first valid Chainlink round with `updatedAt > expiry`: the hint is verified as the first round after expiry by position (with `prev` as in §9.3) and then up to `MAX_GARBAGE_SKIP` successors with `answer ≤ 0` are skipped, so a garbage round published right after expiry cannot remove the permissionless path (D-057). Any lateness, `oraclePaused() == false`. **Clamps** the answer into the resolution band, sets `settlementPrice` to the clamped value, `settlementPath = 5`, emits `SeriesResolvedByOracle(seriesId, rawAnswer, clampedPrice, roundId)` and finishes settlement exactly as `settle` would. Clamping rather than rejecting guarantees that a halted series can always be resolved without any key, which removes the single-admin-key liveness dependency; the timelock path stays for feeds that never return. Before `haltedTimeout` only the timelock can resolve.
...
- Payment modes (per vault, set by the curator, effective after timelock):
  ...
  - `WRITE` (post-token, founder decision, reading confirmed 2026-09-02): the USDG fee is still deducted from premium at clearing; the FeeRouter then debits the curator's prefunded WRITE balance by `feeUSD × (1e4 − writeDiscountBps) / 1e4` converted at the WRITE/USD price supplied by `WritePriceOracle` (Chainlink when a feed exists and is fresh, else the 30-minute WRITE/USDG pool TWAP with the depth rule, the USDG peg check and the mandatory sanity band, D-064…D-068) (**gated on `writeToken` and `priceOracle`, both `address(0)` until the token launches and set by timelock post-launch; D-084 supersedes D-016's `writePool` flag**; while either is zero, `setFeeMode(WRITE)` reverts; `depositWrite` is gated on `writeToken` and on a curator being set, so a deposit can never be stranded without a withdrawal path, D-098), burns `writeBurnShareBps` of it and forwards the rest to `treasury`; the USDG fee is then paid out to `curatorOf[vault]` (D-086). If the curator's WRITE balance is insufficient the router falls back to the USDG path for that clearing and emits `WriteModeFallback`. **`writeDiscountBps = 2000` (20 % discount), `writeBurnShareBps = 5000` (50 % of the WRITE burned, rest to treasury)** — founder decision D-011.
- `depositWrite(vaultId, amount)` (permissionless top-up) / `withdrawWrite(vaultId, amount)` (curator only); withdrawals allowed any time (no lock).
- Implemented v0.6 (`contracts/src/FeeRouter.sol`): the whole WRITE path runs inside `flush`, never inside `collect`, which stays pure bookkeeping — so no oracle read, burn or transfer can reach the permissionless `clear` even through a gas bomb (D-085). Every WRITE-path failure (token or oracle unset, no curator, no price, insufficient prefunded WRITE) degrades to the USDG path and emits `WriteModeFallback(vault, reason)`. Both conversion legs round up (D-088). `writeDiscountBps` and `writeBurnShareBps` are global and bounded by 5000 / 10000 (D-087).
...
- `SAFETY_MODULE`: `globalCapUSD = k × safetyModuleValueUSD`, `k` default 5 (WAD-scaled, bound `[1, 20]`), `safetyModuleValueUSD = SafetyModule.totalStaked() x WRITE/USD price (30-min TWAP, or Chainlink when a feed exists)`, exposed as `SafetyModule.valueUSD()` and, under SPEC's own name, `safetyModuleValueUSD()`. Per-vault cap = `globalCapUSD × capWeightBps[vaultId] / 1e4`, weights set by timelock (governance), `Σ weights ≤ 1e4`; **default at switch-over is an equal split** `floor(1e4 / activeVaults)` across active vaults (founder decision D-011). A vault may also keep a `capUSD` ceiling under this mode (`min` of both).
- Caps limit deposits only; they never force withdrawals.
- Implemented v0.6: `SafetyModule` supplies `valueUSD()` (6 dec) via the shared `WritePriceOracle`. Two gates on that price close every deposit cap protocol-wide until governance acts, and both are deliberate: a WRITE quote outside `[sanityLow8, sanityHigh8]` is reported as unavailable rather than clamped (D-066), and the pool TWAP is rejected while the L2 sequencer is down or inside its recovery grace (D-098, T-05). An unavailable price returns **0 rather than reverting** (D-069), so `vaultCapUSD` stays readable while the cap closes; the vault then rejects deposits with OpenZeppelin's `ERC4626ExceededMaxDeposit`, not `CapPriceUnavailable`.
...
- Migration to WRITE: `bondAsset` switch by timelock (`startMigration(graceSeconds)`, D-081) with a grace window — 30 days intended, bounded on-chain by `MAX_GRACE = 90 days`, extendable but never shortened — during which both assets satisfy the requirement; after grace, USDG bonds no longer count and become withdrawable immediately (no cooldown).
...
- Implemented v0.4 (`contracts/src/BondManager.sol`): `postBond(kind)` tops the bond up to `requiredAmount[kind]` (so a raised requirement never strands a holder) and reverts while a withdrawal is pending; `requestWithdraw` needs zero active locks and starts the 7-day cooldown, during which `hasActiveMMBond` is false; `cancelWithdraw` restores it; `withdrawBond` needs the cooldown elapsed and still zero locks; `lock`/`unlock` are AuctionHouse-only and idempotent (`isLocked[holder][seriesId]`, `activeLocks[holder]`, gating the MM bond only, D-049); `slashBond` is `onlyOwner`, capped at the bond, proceeds to `treasury`, and a slashed bond below the requirement no longer counts until topped up. `bondManager.auctionHouse` is set once after deployment (D-044). Per-series curator locks are not implemented yet. The WRITE migration **is** implemented in v0.6: a holder keeps one independent bond leg per asset (balance, cooldown and requirement all per-asset, D-079), the WRITE requirement is a fixed token amount and never oracle-denominated (D-080), and the migration is four separately visible timelock calls (D-081). During grace both legs qualify; after grace a de-accepted leg skips the cooldown but **not** the `activeLocks` check, and the lock gates qualification rather than a named asset, so a locked MM may pull the leg that is not backing its live series but can never withdraw the collateral standing behind one (D-082). `usdg` stays `immutable`, so `AuctionHouse`'s constructor assert is unaffected. `isBonded(account, kind)` is the general form; `hasActiveMMBond` is its MM specialisation.

---

## 14. Safety module (post-token)

- `stake(amount)` credits non-transferable internal shares: at a constant `1e6` shares per WRITE at genesis (the ERC-4626-style virtual offset, `DECIMALS_OFFSET = 6`, so the first staker's precision is kept and a donation cannot move the share price), pro rata after any slash (**amended by D-071**; a literal 1:1 `sWRITE` peg breaks the instant a slash lands). `requestUnstake(amount)` starts a **14-day cooldown**; `unstake()` is valid in a 3-day claim window after the cooldown; missing the window requires a new request. Stake in cooldown is still slashable.
- `slash(amount, recipient, evidenceURI)` — timelock only, **≤ 30 % of `totalStaked()` per event**, and at most one slash per 14 days (**amended by D-079**: the interval is global rather than per vault incident, which is stricter). Slashed WRITE goes to a timelock-chosen `recipient` (**amended by D-098**: there is no `ShortfallReserve` type or allowlist; the only constraint is that it is neither the zero address nor the module itself, since the latter would strand the amount where no credit can reach it). A slash also voids every unstake request that had already matured, so a standing request cannot be used to sit one block from an exit (D-098).
- Shortfall flow: `ShortfallRecorded(seriesId, tokensShort)` (§9.7 step 5, or an exploit) → timelock proposal that slashes, converts WRITE to the needed asset (Uniswap or OTC; executed by the timelock as a second proposal) and calls `vault.injectCoverage(seriesId, tokens)` which raises `payoutOwed` back to full and re-enables claims at 100 %. Implemented v0.7 (D-099): `injectCoverage` is `onlyOwner` (the timelock), accepts only a SETTLED or RESOLVED series, and raises `payoutPerOption` toward the **unscaled** payout recomputed from the stored `settlementPrice` and `strike` (§10.2 makes that exact for the life of the series, so nothing new is stored). Only the options still unclaimed (`filledQty − claimedQty`) are restored: holders who already claimed were paid at the scaled rate and there is no per-holder ledger to top them up. `tokens` is an **upper bound**, not an exact amount — the call pulls only what the raise makes claimable and clamps at 100 %, so an oversized OTC fill still executes instead of costing another 48 h round trip. The balance and `payoutOwed` rise by the same amount, so `totalAssets()` and the share price never move: coverage reaches option holders, never depositors. `vault.coverageNeeded(seriesId)` sizes the purchase.
...
- Implemented v0.6 (`contracts/src/SafetyModule.sol`, `EmissionsController.sol`, `WritePriceOracle.sol`). Emissions are **pulled** by the module, never pushed (D-074) — the stake asset is also the reward asset, so `totalStaked` is an explicit accumulator and never `balanceOf`. Emissions accruing while nothing is staked are parked in `unallocatedRewards` and only the timelock can redirect them (D-075), which closes the first-staker-takes-the-backlog bug. `slash` is capped at 30 % per event **and** one event per 14 days (D-079); the 14-day unstake cooldown dominates the 48 h timelock, so no staker can exit ahead of a queued slash. `vault.injectCoverage` ships in v0.7 (D-099), so the slash-to-shortfall loop now closes on chain: slashed WRITE lands at a timelock-controlled address, is converted off-chain, and re-enters the vault as option-holder coverage.
...
- Guardian (`GUARDIAN_ROLE` in RiskModule, no delay): **two holders** (D-012 as amended by D-029): a hot key on the keeper server, distinct from the keeper's transaction key, so the keeper's alerting can pause within seconds without human presence; and an off-server key (hardware wallet or phone signer) held by the founder, so a compromised server cannot prevent a human pause during the 48 h it takes to rotate roles. Functions: `pauseNewAuctions(vault | ALL)`, `pauseDeposits(vault | ALL)`, `unpauseNewAuctions`, `unpauseDeposits` (`ALL = address(0)`; `ALL` dominates the per-vault flag; either holder can unpause the other's pause, including the pause set by a halt once reviewed — the HALTED vault stays closed by its own state machine until resolved, D-050). Compromise of either key can at worst pause new auctions and deposits; rotation is a timelocked `grantRole/revokeRole` (RUNBOOK.md). It cannot: move any funds, change any parameter, block `withdraw/redeem` in IDLE, block `claimPremium`, `withdrawRefund`, `claimOptions`, `OptionToken.claim` or `claimWithdrawal`, block queued-redeem processing after settlement, or stop `settle`, `halt` or `resolveHaltedByOracle`. A deposit pause does hold queued **deposits** (they execute after unpause, §4.3), never queued redeems. After a guardian pause, any live series still settles; once settled the vault is IDLE with withdrawals open indefinitely until unpause.
- Deployer (**amended by D-103**; the text below described contracts that set the deployer as owner, which none of them do any more): deploys `TickMath` first and links its address into `SettlementOracle` and `WritePriceOracle` (D-058; both constructors reject an unlinked or mis-linked library), then **only calls `new`**. Every contract takes the `TimelockController` as its `owner_` constructor argument, so the deployer never holds a privilege, there is no `renounceRole` step and no ownership transfer at all: the wiring is two timelock batches, and the deploy is four stages (deployer news, batch A wires, deployer news the staking pair, batch B finishes). The post-deploy checklist (`increaseObservationCardinalityNext`, verify on Blockscout) is unchanged. Encoded in `contracts/script/` and driven by `contracts/config/<chainId>.json` (D-101); the procedure is `docs/RUNBOOK.md`. Verified by `contracts/script/Verify.s.sol`, which reads every role, owner, wiring, cap, fee, bond and pause flag off a deployed system and can be run standalone, and by the fork test `contracts/test/DeployFork.t.sol`, which runs the same deploy library and the same assertions against a fork. No contract that holds user funds has a sweep, rescue or arbitrary-call function.
...
- `ShortfallRecorded(*seriesId, tokensShort)`, `CoverageInjected(*seriesId, tokens)` (`tokens` is what was actually pulled after the clamp, §14)
OptionToken
- `VaultRegistered(*underlying, *vault)`, `SeriesCreated(*id, *vault, *underlying, kind, strike, expiry, multiplierAtCreation)`, `SeriesSettled(*id, settlementPrice, payoutPerOption)`, `PayoutRaised(*id, payoutPerOption)` (mirror follows `injectCoverage`, §14), `OptionClaimed(*seriesId, *holder, *to, qty, tokens)`
Series / Auction
...
- vault: `SeriesSettled(*seriesId, settlementPrice, settlementPath, payoutPerOption, payoutTotal)`; oracle: `Settled(*seriesId, path, price8, roundId)`
- `PathRejected(*seriesId, path, reason)` (implements the earlier `TwapRejected`; `reason` ∈ `GRACE`, `OBSERVE`, `LIQUIDITY`, `OBSERVATIONS`, `USDG_STALE`, `USDG_OUT_OF_BAND`, `BOUND`, `NO_ROUND`, `STALE`, `INVALID_ANSWER`, `DEADLINE`), `JumpGuardTripped(*seriesId, path, price)`, vault `SeriesHalted(*seriesId, reason)` and oracle `SeriesHalted(*seriesId, reason, resolveRef)`, `SeriesResolved(*seriesId, price, evidenceURI)`, `SeriesResolvedByOracle(*seriesId, rawAnswer, clampedPrice, roundId)`
...
- `FeeCollected(*vaultId, *seriesId, usdg, mode)`, `FeeFlushed(*vault, *to, usdg)` (in WRITE mode `to` is the curator, not the treasury), `VaultInitialised(*vault, feeBps)`, `WriteFeePaid(*vault, *curator, feeUSDG, writeAmount, burned, price8)`, `WriteModeFallback(*vault, reason)`, `WriteDeposited(*vault, *from, amount)`, `WriteWithdrawn(*vault, *to, amount)`, `WriteTokenSet(*writeToken)` (D-089)
- `BondPosted(*holder, kind, asset, amount)`, `BondLocked(*holder, *seriesId)`, `BondUnlocked(*holder, *seriesId)`, `BondWithdrawRequested(*holder, kind, asset, unlockAt)`, `BondWithdrawCancelled(*holder, kind, asset)`, `BondWithdrawn(*holder, kind, asset, amount)`, `BondSlashed(*holder, kind, asset, amount, evidenceURI)`, `MigrationStartedAt(from, to, graceEndsAt)`, `AuctionHouseSet(*auctionHouse)` (BondManager and FeeRouter, once)
- `Staked(*account, assets, shares)`, `UnstakeRequested(*account, shares, unlockAt)`, `UnstakeCancelled(*account, shares)`, `Unstaked(*account, shares, assets)`, `RewardsClaimed(*account, amount)`, `RewardsAccrued(amount, accRewardPerShare)`, `RewardsUnallocated(amount, total)`, `UnallocatedRedirected(*to, amount)`, `Slashed(*recipient, amount, evidenceURI)`
- Token layer (v0.6): `ScheduleCreated(*id, *beneficiary, totalAmount, start, cliff, end)`, `Released(*id, *beneficiary, amount)`, `Revoked(*id, vested, returnedToPool)`, `Reallocated(*to, amount)`, `BeneficiaryProposed(*id, *from, *to)`, `BeneficiaryChanged(*id, *from, *to)`, `RoundSet(*roundId, root, amount, start, deadline)`, `Claimed(*roundId, *index, *account, amount)`, `Swept(*roundId, *treasury, amount)`, `PoolSet(*pool)` (LiquidityEscrow) and `PoolSet(*pool, writeIsToken1, fee)` (WritePriceOracle), `Funded(*pool, amount, released)`, `Emitted(*sink, amount, released)`, `SinkSet(*sink)`, `ChainlinkFeedSet(*feed, decimals)`
...
- hints are built off-chain from the feed and validated with `oracle.previewSettle(seriesId, hint)` and `oracle.previewResolveByOracle(seriesId, roundId, prevRoundId) → (ok, price8)`, which mirror `settle` and `resolveHaltedByOracle` exactly (D-057); `oracle.canHalt(seriesId, hint) → (bool, reason)`; `oracle.twap(vault, window) → (twapUSDG8, twapUSD8, reason)`; `oracle.vaultConfig(vault)` carries the feed's registered first round
- observation coverage: the keeper reads `slot0.observationCardinality` and the oldest `pool.observations(i)` directly (no oracle helper, v0.5)
...
- `series(seriesId)` struct, `vault.currentSeriesId()` (the next scheduled event is computed off-chain by the keeper from `AuctionHouse.scheduledExpiry`; there is no `nextEvent()` view)
- `cap.vaultCapUSD(vault)`, `cap.remainingDepositAssets(vault, totalAssets) → (assets, priceOk)`, `cap.capMode()`
...
- `oracle.twap(vault, window) → (twapUSDG8, twapUSD8, reason)`, `oracle.previewSettle(seriesId, hint)`, `oracle.canHalt`, `oracle.canResolveByOracle`, `oracle.previewResolveByOracle`, `oracle.resolutionBand` for the settlement-preview panel (`lastChainlink` and `usdgUsd` were dropped by D-057: the panel reads the feeds directly)
...
- Token layer (v0.6): `SafetyModule.valueUSD / valueUSDView / safetyModuleValueUSD / totalStaked / sharesOf / stakedOf / previewStake / previewUnstake / pendingRewards / rewardSurplus / unstakeWindow`; `WritePriceOracle.writePrice / usdValueOfWrite / writeForUSD / previewPrice`; `EmissionsController.accrued / claimable / undistributed`; `BondManager.isBonded / assetAccepted / assetToken / requiredAmountOf / statusIn / migrationEndsAt`; `FeeRouter.writeBalance / previewWriteFee / curatorOf`; `Vesting.vestedAmount / releasable / unallocated / schedules / idsOf`; `PointsDistributor.leafHash / isClaimed / rounds / unreserved`.
...
- **I-2 Payout bound**: `payoutPerOption < 1e18` for every settled series; `payoutOwed ≤ asset.balanceOf(vault)` unless a `ShortfallRecorded` event exists, and again after every `CoverageInjected`. `payoutPerOption` never exceeds the unscaled `(S − K) × 1e18 / S`, and `OptionToken`'s mirrored copy always equals the vault's (§14).
...
- **I-17 Share ledger**: `Σ sharesOf == totalShares`; shares are non-transferable and never leave the ledger.
- **I-18 Claims never reach principal**: `write.balanceOf(SafetyModule) ≥ totalStaked + unallocatedRewards` at all times, so a reward claim can never be paid out of staked principal or parked emissions.
- **I-19 Principal is tracked, not measured**: whenever rewards exist, `totalStaked < write.balanceOf(SafetyModule)` — the accumulator is never the balance, which is what keeps a slash from consuming rewards.
- **I-20 Slash cap**: no slash above 30 % of `totalStaked` ever succeeds, proven by an action that deliberately attempts one on every call.
- **I-21 Shares and principal vanish together**: `totalShares == 0 ⇔ totalStaked == 0`.
- **I-22 Rewards are funded**: everything owed to stakers (including unharvested index movement) is bounded by what `EmissionsController` actually released, plus at most one wei per harvest of index dust (D-097).
- **I-28 Owed ≤ surplus**: everything owed is covered by `rewardSurplus()`, plus the same dust.
- **I-29 Principal conservation**: `stakedIn − unstakedOut − slashedOut == totalStaked`.
- **I-30 Donations are inert**: WRITE sent directly to the module raises its balance but creates no credit.
- **I-23 Vesting allocation**: `totalAllocated == Σ schedule.totalAmount`, preserved by `revoke` (which shrinks both).
- **I-24 Vesting buckets partition**: `unallocated + totalAllocated + reallocated == allocation`, derived from the immutable so a donation cannot move it.
- **I-25 Released ≤ vested**: per schedule, never in aggregate only.
- **I-26 Outstanding promises are funded**: `totalAllocated − totalReleased ≤ balanceOf(Vesting)`.
- **I-27 Release bookkeeping**: `totalReleased == Σ schedule.released`.
- **I-31 Fixed supply**: `totalSupply == MAX_SUPPLY − Σ burned`; there is no mint path.
- **I-32 Points buckets partition**: `paidOut + swept + outstanding + unreserved == allocation`.
- **I-33 Live rounds are funded**: `balanceOf(PointsDistributor) ≥ outstanding`.
- **I-34 No round overpays**: `Σ round.claimed ≤ Σ round.amount`, and `paidOut` equals the former.
- **I-35 Escrow bound**: `released ≤ allocation`, the remainder is still held, and everything released reached `pool`.
- **I-36 Fee conservation (WRITE mode)**: `Σ collected == Σ flushed + pending`, whichever mode each flush took.
- **I-37 Prefunded WRITE conservation**: `deposited == held + withdrawn + burned + toTreasury` — there is no fifth destination.
- **I-38 Router holdings**: `write.balanceOf(FeeRouter) == Σ writeBalance`, and the router never holds USDG.
- **I-39 Bond legs are backed**: per asset, `Σ legs ≤ balanceOf(BondManager)` in every state of the migration.
- **I-40 Bonded implies the requirement is met**: a qualifying account always has an accepted leg at or above its own asset's requirement with no pending withdrawal.
- **I-41 The router only ever burns**: WRITE mode never mints; supply falls only by the burn share.

Encoded (`contracts/test/invariants/`): `SafetyModuleInvariants.t.sol` (I-17…I-22, I-28…I-30; the handler stakes, requests, warps deliberately into the 3-day claim window — a blind walk reaches it ~0.3 % of the time — cancels, claims, pokes, donates, slashes, and attempts an over-cap slash on every call so I-20 is proven by the contract rather than restated by the handler). `VestingInvariants.t.sol` (I-23…I-27, with a donation action). `TokenDistributionInvariants.t.sol` (I-31…I-35, driving Merkle rounds, claims, sweeps, escrow funding, burns and donations). `WriteFeeBondInvariants.t.sol` (I-36…I-41; the auction suite runs on `AuctionBaseTest`, where the token and the migration are unreachable, so this is the only coverage of WRITE mode and the dual-asset ledger, D-097). Each suite carries a deterministic `test_handlerReachesEveryState`.
...
Auction layer (`contracts/test/invariants/AuctionInvariants.t.sol`, v0.4; the handler plays depositors, four bonded MMs, the keeper, the settlement oracle, the issuer, Paxos freezes and the timelock, with structured time travel): I-3 in all three forms (`invariant_I3_escrowExact`, `invariant_I3_closedConservation`, `invariant_I3_allocationIdentity`, the last one also asserting `filledQty ≤ offeredQty`, `clearingPrice ≥ reservePrice` and vault/auction agreement), I-13 (`invariant_I13_bondLocks`: a filled bidder of the LIVE/HALTED series is locked and no bond withdrawal ever succeeded while locked), fee conservation (`pending + treasury == Σ fee`), `previewClear == clear`, and "auction OPEN ⇔ vault AUCTION" (the skip paths of D-045 never leave the vault stuck). Settlement layer (`contracts/test/invariants/SettlementInvariants.t.sol`, v0.5; the handler plays the keeper, MMs, the Chainlink operator with phase bumps and garbage rounds, the pool with swaps and liquidity pulls, the USDG/USD feed, the issuer with splits, both guardians with a `vm.record` storage check and the timelock with bounded parameter versions and in-band resolutions): `invariant_I5_neverStale`, `invariant_I7_guardianScope`, `invariant_I9_usdgBand`, `invariant_I10_jumpGuard`, `invariant_I11_resolutionBand`, `invariant_I12_paramSnapshot`, `invariant_I14_expiredSeriesActionable` (past the backstop, the hint an honest keeper builds makes `settle` or `halt` succeed and neither view reverts — the arm that catches a series stuck LIVE, D-057) and `invariant_I14_liveness` (every call the views approved succeeded; a halted series past the timeout with a fresh post-expiry round is resolvable without a key), `invariant_payoutsBounded` (per series Σ claimed ≤ filledQty × payoutPerOption / 1e18 ≤ encumbered collateral and ≤ totalAssets at settlement; `payoutOwed` backed), `invariant_previewMatchesSettle`, `invariant_haltedVaultCannotOpen`; `test_handlerReachesEveryState` reaches paths 1–5 and two halts deterministically.
...
OQ-004 (2026-09-03, D-047): **closed by D-050/D-054** — both `priceSource` setters are pointed at SettlementOracle at deployment and the `OracleParams` snapshot lives in RiskModule versions selected by `auctionOpen`; freezing the two setters (a redeploy) stays with OQ-005.

OQ-005 (2026-09-03, D-054): the AuctionHouse derives `S_ref` from `capPrice` (§12: Chainlink < 80 h, no `oraclePaused` check) instead of `referencePrice` (§7.2: Chainlink ≤ 26 h and not paused, else TWAP within 15 %). Switching needs an AuctionHouse redeploy (1.4 KB of headroom). The freeze half of this question is **closed by D-113**: both `priceSource` setters are disabled by `freezePriceSource` in deploy batch A, so the switch is a redeploy of AuctionHouse alone.
