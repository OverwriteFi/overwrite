...
- `stake(amount)` credits non-transferable internal shares: 1:1 at genesis, pro rata after any slash (**amended by D-071**; a literal 1:1 `sWRITE` peg breaks the instant a slash lands). `requestUnstake(amount)` starts a **14-day cooldown**; `unstake()` is valid in a 3-day claim window after the cooldown; missing the window requires a new request. Stake in cooldown is still slashable.
...
OQ-005 (2026-09-03, D-054): the AuctionHouse derives `S_ref` from `capPrice` (§12: Chainlink < 80 h, no `oraclePaused` check) instead of `referencePrice` (§7.2: Chainlink ≤ 26 h and not paused, else TWAP within 15 %). Switching needs an AuctionHouse redeploy (1.5 KB of headroom); do it together with freezing `AuctionHouse.priceSource` / `CapController.priceSource`.
