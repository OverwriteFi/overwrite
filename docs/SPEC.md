...
- Payment modes (per vault, set by the curator, effective after timelock):
  ...
  - `WRITE` (post-token, founder decision, reading confirmed 2026-09-02): the USDG fee is still deducted from premium at clearing; the FeeRouter then debits the curator's prefunded WRITE balance by `feeUSD × (1e4 − writeDiscountBps) / 1e4` converted at the WRITE/USDG 30-minute TWAP (**gated on `writeToken` and `priceOracle`, both `address(0)` until the token launches and set by timelock post-launch; D-084 supersedes D-016's `writePool` flag**; while either is zero, `setFeeMode(WRITE)` reverts and `depositWrite` is disabled), burns `writeBurnShareBps` of it and forwards the rest to `treasury`; the USDG fee is then paid out to `curatorOf[vault]` (D-086). If the curator's WRITE balance is insufficient the router falls back to the USDG path for that clearing and emits `WriteModeFallback`. **`writeDiscountBps = 2000` (20 % discount), `writeBurnShareBps = 5000` (50 % of the WRITE burned, rest to treasury)** — founder decision D-011.
...
- Implemented v0.6: `SafetyModule` supplies `valueUSD()` (6 dec) via the shared `WritePriceOracle`. An unavailable price returns **0 rather than reverting** (D-069), so `vaultCapUSD` stays readable while the cap closes; the vault then rejects deposits with OpenZeppelin's `ERC4626ExceededMaxDeposit`, not `CapPriceUnavailable`.
...
- `FeeCollected(*vaultId, *seriesId, usdg, mode)`, `FeeFlushed(*vault, *treasury, usdg)`, `VaultInitialised(*vault, feeBps)`, `WriteFeePaid(*vault, *curator, feeUSDG, writeAmount, burned, price8)`, `WriteModeFallback(*vault, reason)`, `WriteDeposited(*vault, *from, amount)`, `WriteWithdrawn(*vault, *to, amount)`, `WriteTokenSet(*writeToken)` (D-089)
