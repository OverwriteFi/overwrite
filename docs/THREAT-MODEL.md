...
**Scenario.**
1. **Hints.** `settle(seriesId, hint)` is permissionless. Hints are verified, so a front-runner cannot make the contract accept a different round than the one that satisfies the rules. Two callers with different hints produce the same result or one reverts. **Until the 2026-09-05 audit this was false for `obsIndex`** (audit R-1): the observation hint was advisory ("an older hint only under-counts"), so from the moment Monday's first round existed anyone could pass the live head, fail the TWAP with `OBSERVATIONS`, and settle a WEEKEND series on path 3 — the exact free choice this threat describes, bounded only by the 30 % jump guard. `_observationsOk` now verifies the hint (newest observation at or before the anchor) and reverts `BadObsHint` otherwise; `test_T08_liveHeadHintCannotForcePath3`. The same audit closed a phase-overlap variant for round hints (R-2, SPEC §9.1).
...
- Tests: `SettlementOracle.t.sol::test_T08_differentValidHintsSameResult`, `test_T08_wrongHintReverts`, `test_T08_path2PreferredWhenBothValid`, `test_T08_lastChainlinkFlipInsideGrace` (documents the reference flip), `test_T08_liveHeadHintCannotForcePath3`, `test_path2_observationHintCannotHelp` (wrong hints revert), `test_path1_phaseOverlapHasOneReference`, `Vault.t.sol::test_T08_withdrawAfterSettleCannotDodgePayout`.
...
- Tests: `CoveredCallVault.threats.t.sol::test_T11_settleSucceedsWhileTokenPaused` (mock token `pause()`), `::test_T11_mintSeriesRevertsAfterIssuerBurnBetweenOpenAndClear`, `::test_T11_injectCoverageRestoresFullPayout`, `CoveredCallVault.t.sol::test_settle_shortfallScalesPayout` (the burn creates the shortfall and claims scale pro rata) and its `test_injectCoverage_*` neighbours, `DayInTheLife.t.sol::test_e2e_slashCoversAShortfall` (slash to restored payout, D-099), invariant handler with `issuerBurn` and `injectCoverage` actions asserting **I-2**'s "unless ShortfallRecorded" clause and its restoration; fork test reading `paused()`, `oraclePaused()`, beacon slot and implementation address for every allowlisted token at deploy (`Deploy.t.sol::test_T11_tokenSurfaceMatchesSpec`).
...
**Amended 2026-09-05 (audit F-2, D-113).** A keeper that is merely *down* — no key compromise needed — used to hand every bidder a free option: `clear` was callable by anyone from `auctionClose` until `expiry`, bids cannot be cancelled and depositors cannot exit during AUCTION, so a bidder could wait up to five days, clear only if the stock had rallied past the strike (an ITM option for an OTM premium) and let the auction skip otherwise. `AuctionHouse.clearGrace` (default 1 h, timelocked within [5 min, 4 h]) now turns a late clear into the skip path; a skipped week costs one premium, a late clear could cost most of the collateral's upside. `test_clear_afterGraceSkips`. The original T-13 text follows.
...
3. **Delay reduction.** `TimelockController.updateDelay` is self-administered: the attacker queues a delay change to 0 (48 h), then acts at will. Still 48 h of visibility for the first step. The same class: every owned contract keeps `Ownable2Step.transferOwnership`, so one 48 h step can hand a contract to an undelayed key (the new owner must `acceptOwnership`, which is visible too). Both are accepted residuals of D-003; the mitigation is the one that exists for the whole class — watch the timelock queue.
3b. **Re-pointing the price source (closed 2026-09-05, D-113, audit G-1).** Before the freeze, one 48 h batch could point `AuctionHouse.priceSource` at a contract answering `capPrice = 400` and grant the attacker `KEEPER_ROLE`; the next series then had a strike of ≈ $0.000004 and reserve bounds of a few USDG units, the attacker bought 100 % of `offeredQty` for dust, every settlement path tripped the jump guard, and the halt resolved at ≈ the real price → `payoutPerOption ≈ 1e18`, i.e. the whole unencumbered collateral, far above the ≈ 16 % bound of item 2. `freezePriceSource` on both consumers (executed in deploy batch A) removes the lever; the residual is again the redeploy-and-migrate class of T-17.
...
**Enforced by.**
- `nonReentrant` on every state-changing external function of `CoveredCallVault`, `AuctionHouse`, `OptionToken.claim` (added in the 2026-09-05 audit pass, D-113; before that only the vault's `payOptionClaim` guard stood behind it), `BondManager` (holder functions; `lock`/`unlock` are `onlyAuctionHouse` and make no external call), `FeeRouter` (`collect` is `onlyAuctionHouse` bookkeeping), `SafetyModule`, `Vesting.release`, `PointsDistributor.claim/sweep`, `LiquidityEscrow.fund`, `SettlementOracle.settle/halt/resolve*`. Cross-contract: a single `ReentrancyGuardTransient`-style lock shared per vault (the vault exposes `lock/unlock` callable only by `AuctionHouse` and `SettlementOracle`) or, simpler, all mutating flows enter through the vault which then calls out. Effects before interactions everywhere; ERC-1155 mint at the very end of `clear` (after premium, fee, refunds are recorded) and preferably with the receiver check moved to `bid` time (RS-07).
...
**Enforced by (RS-16 as amended by D-032).** `openAuction` never requires an empty queue: `openSeries` executes up to `maxQueueOpsPerOpen` requests itself and opens regardless of what is left, and every visited entry (including cancelled ones) counts against the bound, so no queue shape can revert or exhaust it. `requestDeposit` is therefore allowed in every state (the earlier "reverts in IDLE" rule was dropped with D-032 as unnecessary once the open no longer depends on the queue). Tests: `CoveredCallVault.threats.t.sol::test_T18_*`, `test_T07_settleGasAtMaxQueueOps`, fuzz on queue lengths.
